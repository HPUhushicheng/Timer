  ...
  const fetchData = async () => {
    try {
      ...
      timeData.forEach(record => {
        if (!userTimeStats[record.id]) {
          userTimeStats[record.id] = 0;
        }
        userTimeStats[record.id] += (Number(record.hourtime) / 60) || 0;
      });

      // 将用户信息和时长关联
      const combinedData = users.map(user => ({
        name: user.name,
        totalTime: Number((userTimeStats[user.id] || 0).toFixed(1))
      }));
